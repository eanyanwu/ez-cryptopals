...
/// Convert a hexadecimal string to underlying sequence of binary numbers
pub fn ascii_hex_to_bytes(hex_text: &str) -> Vec<u8> {
...
pub fn bytes_to_ascii_base64(bytes: &[u8]) -> String {
    let base64_ascii_characters = bytes.chunks(3)
            .flat_map(|bytes| {
                if bytes.len() == 3 {
                    ...
                    vec![
                        byte_to_ascii_base64(&first_sextet),
                        byte_to_ascii_base64(&second_sextet),
                        byte_to_ascii_base64(&third_sextet),
                        byte_to_ascii_base64(&fourth_sextet)
                    ]
                }
                else if bytes.len() == 2 {
...
fn ascii_hex_to_byte(hex_char: &u8) -> u8 {
    ...
    match hex_char {
        ...
        48..=57 => hex_char - START_ASCII_DIGIT,

        // Range of ascii hexadecimal "alphabet" characters
        65..=70 => hex_char - START_ASCII_UPALPHA + 10, // lowercase
        97..=102 => hex_char - START_ASCII_LOALPHA + 10, // uppercase
...
fn byte_to_ascii_base64(num: &u8) -> u8 {
    match num {
...
pub mod challenge01 {
    ...
    pub fn convert_hex_to_base64() {
        ...
        let bytes = set01::ascii_hex_to_bytes(hex_input);
        let actual_base64_output = set01::bytes_to_ascii_base64(&bytes);
    ...
    pub fn test_ascii_hex_to_byte() {
        assert_eq!(0, set01::ascii_hex_to_byte(&b'0'));
        assert_eq!(9, set01::ascii_hex_to_byte(&b'9'));

        assert_eq!(10, set01::ascii_hex_to_byte(&b'a'));
        assert_eq!(10, set01::ascii_hex_to_byte(&b'A'));

        assert_eq!(15, set01::ascii_hex_to_byte(&b'f'));
        assert_eq!(15, set01::ascii_hex_to_byte(&b'F'));
    }
    ...
    {
        assert_eq!(vec![0x01,0x23,0x45,0x67,0x89], set01::ascii_hex_to_bytes("0123456789"));
        assert_eq!(vec![0xab,0xcd,0xef], set01::ascii_hex_to_bytes("abcdef"));
        assert_eq!(vec![0x01, 0x23], set01::ascii_hex_to_bytes("123"));
    }

    #[test]
    pub fn test_byte_to_ascii_base64() {
        assert_eq!(b'A', set01::byte_to_ascii_base64(&0));
        assert_eq!(b'Z', set01::byte_to_ascii_base64(&25));

        assert_eq!(b'a', set01::byte_to_ascii_base64(&26));
        assert_eq!(b'z', set01::byte_to_ascii_base64(&51));

        assert_eq!(b'0', set01::byte_to_ascii_base64(&52));
        assert_eq!(b'9', set01::byte_to_ascii_base64(&61));

        assert_eq!(b'+', set01::byte_to_ascii_base64(&62));
        assert_eq!(b'/', set01::byte_to_ascii_base64(&63));
    }
    ...
    pub fn test_bytes_to_ascii_base64() {
        ...
        assert_eq!(String::from("TWFu"), set01::bytes_to_ascii_base64(&[b'M', b'a', b'n']));
        assert_eq!(String::from("TWE="), set01::bytes_to_ascii_base64(&[b'M', b'a']));
        assert_eq!(String::from("TQ=="), set01::bytes_to_ascii_base64(&[b'M']));
    }
}